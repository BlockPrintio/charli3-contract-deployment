 ...
 * Each call to oracleManager / oracleNfts produces the script CBOR,
 * its hash, and (for spending scripts) the derived script address.
 */
export class OracleScripts {
   ...
   * utxoRef — the specific UTxO consumed in the bootstrap tx (unique policy ID per instance).
   */
  async oracleNfts(
    ...
    const nftsConf = conStr0([
      ...
      toOracleConfigData(conf),
      byteString(managerHash),  // oracle_script_hash: ScriptHash (ByteArray)
    ]);
